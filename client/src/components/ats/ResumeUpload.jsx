...
/**
 * Resume upload widget for the ATS Checker.
 *
 * Props:
 *   file        — currently selected File object (new upload), or null
 *   onChange    — called with a File when user picks one, or null when removed
 *   savedResume — { name, uploadedAt } from DB, shown when no new file is selected
 */
const ResumeUpload = ({ file, onChange, savedResume }) => {
  ...
  // No new file, but a saved resume exists — show it as pre-loaded
  if (savedResume) {
    const uploadedDate = savedResume.uploadedAt
      ? new Date(savedResume.uploadedAt).toLocaleDateString(undefined, {
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.25 }}
          className="flex-1 min-h-[280px] flex flex-col items-center justify-center border-2 border-emerald-400 dark:border-emerald-600 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 p-6"
        >
          ...
          <button
            ...
            className="mt-4 flex items-center gap-1.5 text-sm text-blue-500 hover:text-blue-600 transition-colors"
          >
            <Upload size={14} />
  ...
  // No file and no saved resume — show the drop zone
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3 flex items-center gap-2">
        <FileText size={20} className="text-blue-500" />
